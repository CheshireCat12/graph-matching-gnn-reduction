...
from graph_pkg_core.algorithm.knn cimport KNNClassifier
...
class RunnerKNN(Runner):

    def __init__(self, parameters, logger):
        super(RunnerKNN, self).__init__(parameters)
        self.logger = logger
    ...
    def evaluate(self, best_params):
        ...
        # current_folder = self.parameters.folder_results
        filename = f'predictions.npy'
        ...
        if self.parameters.save_dist_matrix:
            self.save_ged_distances('knn', knn.current_distances)


cpdef void run_knn(parameters, logger):
    run_h_knn_ = RunnerKNN(parameters, logger)
    run_h_knn_.run()
